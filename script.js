...
/**
 * Parses song data from song.js format to app format
 * @param {object} song - Song object from songData
 * @returns {array} - Array of chord objects with bar, chord, next, etc.
 */
function parseSong(song) {
    const chords = [];
    song.progression.forEach((item, index) => {
...
function loadSong(songId) {
    ...
    // Update Title
    const elTitle = document.getElementById("song-title-display");
    if (elTitle) elTitle.textContent = song.title;

    // Parse Chords
    currentSongChords = parseSong(song);

    // Set Key
    // We try to match the song's key with our available keys
    // Our KEYS object has offsets from Gm. This might be tricky if the song is in a key not relative to Gm easily.
    // However, the app seems to treat "Key" as a transposition interval.
    // If we want to play in the original key of the song, we should probably set currentKey to "Gm" (offset 0) 
    // IF the chords in song.js are already in that key.
    // song.js chords ARE in the key specified in song.js.
    // So if we select "Gm" (offset 0), we play the chords as written.
    // So we should reset transposition to 0 (Gm in the UI).

    currentKey = "Gm";
    const elKeySelect = document.getElementById("key-select");
    if (elKeySelect) {
        elKeySelect.value = "Gm";

        // Update Key Selector Labels based on Song Key
        const match = song.key.match(/^([A-G][#b]?)(.*)$/);
        if (match) {
            const root = match[1];
            const suffix = match[2];

            Array.from(elKeySelect.options).forEach(option => {
                const offset = KEYS[option.value];
                if (offset !== undefined) {
                    const newRoot = transposeNote(root, offset);
                    option.textContent = newRoot + suffix;
                    if (offset === 0) option.textContent += " (Original)";
                }
            });
        }
    }
    ...
    const totalBars = currentSongChords.length;
    loopStart = 1;
    loopEnd = totalBars;
    ...
    if (elLoopStart) {
        elLoopStart.max = totalBars;
        elLoopStart.value = 1;
    }
    if (elLoopEnd) {
        elLoopEnd.max = totalBars;
        elLoopEnd.value = totalBars;
    }
...
function getTransposedChords() {
    ...
    if (interval === 0) return currentSongChords;

    return currentSongChords.map(c => {
        let newChord = { ...c };
...
// Initialize
function init() {
    // Song Search Modal Logic
    const btnSongSearch = document.getElementById("btn-song-search");
    const searchModal = document.getElementById("song-search-modal");
    ...
    function renderSongList(filterText = "") {
        ...
        filteredSongs.forEach(song => {
            ...
            item.innerHTML = `
                ...
                <span class="song-item-meta">${song.style} - ${song.key}</span>
            `;
    ...
    if (btnSongSearch && searchModal) {
        btnSongSearch.addEventListener("click", () => {
            searchModal.classList.remove("hidden");
            renderSongList(); // Show all initially
            if (searchInput) {
                searchInput.value = "";
                searchInput.focus();
            }
    ...
    // Load Initial Song (Autumn Leaves - ID 1)
    loadSong(1);
    // generateChordChart and updateUI are called inside loadSong
    ...
    if (elLoopStart) {
        elLoopStart.addEventListener("change", (e) => {
            ...
            const max = currentSongChords.length || 32;
            if (val < 1) val = 1;
            if (val > max) val = max;
...
function updateUI(bar, beat) {
    ...
    if (nextBeat > 4) {
        nextBar++;
        if (nextBar > totalBars) nextBar = 1;
        nextBeat = 1;
    }
